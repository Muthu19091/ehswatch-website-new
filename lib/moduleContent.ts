...
export function buildModuleTemplateProps(
  ...
  const hero: ModuleTemplateProps["hero"] = {
    ...
    // FE-HO-12: the ONLY source of the highlighted word/phrase is a real
    // <span> the editor wraps it in — headingHtml() preserves and recolours
    // it, the same mechanism Home's headings use. No accent field, no
    // per-module hardcoded default, no last-word fallback.
